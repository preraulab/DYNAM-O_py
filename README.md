...
Companion Rust crate `dynamo_rs` (in `rust/`) provides the region-merging inner loop (`mergeWshedSegment` port). Multitaper spectrogram delegates to the existing [`multitaper_rs`](../../labcode_main/multitaper/rust/) crate.

## Install (development)

```bash
cd ~/code/toolboxes/pydynamo
...
pip install -e ~/code/labcode_main/multitaper/rust/   # or: (cd ~/code/labcode_main/multitaper/rust && maturin develop --release)
```
...
m = sio.loadmat("/Users/Mike/code/toolboxes/DYNAMO_dev/example_data/example_data.mat",
                simplify_cells=True)
...
- `/Users/Mike/code/toolboxes/DYNAMO_dev/example_data/example_data.mat` — bundled example EEG (data, Fs, stage_times, stage_vals)
- `/Users/Mike/code/toolboxes/DYNAMO_dev/segment_out.mat` — saved MATLAB `runDYNAMO('segment')` output
