...
Uses real test data at testdata/. Skipped if absent.
"""
...
EDF_PATH = Path("testdata/example.edf")
STAGING_PATH = Path("testdata/example_staging.csv")

pytestmark = pytest.mark.skipif(
    not EDF_PATH.exists() or not STAGING_PATH.exists(),
    reason="test data not present at testdata",
)
...
def test_matlab_cache_parity_if_available():
    ...
    candidates = glob.glob(os.path.expanduser("../DYNAM-O_rs/data_cache/**/*.mat"), recursive=True)
    if not candidates:
