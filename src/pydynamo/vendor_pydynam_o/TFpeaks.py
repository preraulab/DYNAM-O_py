"""Vendored from pyDYNAM-O/dynam_o/TFpeaks.py with these modifications:

  - skimage.future.graph → skimage.graph (API moved in 0.20+)
  - Import only what we need from matplotlib / numpy at top level
  - `edge_weight` updated to the symmetric merge rule (matches current
    MATLAB edgeWeightEqual.m — `j_border_vals = data[j_border]` instead
    of pyDYNAM-O's original `data[i_border]`).
  - `from __future__ import annotations` so the `skimage._sk_graph.RAG`
    type hints don't eagerly resolve against the removed module.
"""
...
import numpy as np
import pandas as pd
# Provide `skimage.future.graph` as alias for older type hints.
from skimage import graph as _sk_graph
sys.modules.setdefault("skimage.future.graph", _sk_graph)
import skimage.future.graph
from skimage import measure, segmentation, future, color, morphology
from skimage.transform import resize

from pydynamo.vendor_pydynam_o.utils import min_prominence, pow2db


def edge_weight(graph_rag: skimage._sk_graph.RAG, graph_edge: tuple, graph_data: np.ndarray) -> float:
    """
    Computes the edge weight between two regions

    Parameters
    ----------
    graph_rag : skimage._sk_graph.RAG
    ...
    # Symmetric merge rule — matches current MATLAB edgeWeightEqual.m.
    # pyDYNAM-O originally had j_border_vals = data[i_border] (asymmetric
    # "bug" that corresponded to the older MATLAB rule).
    i_border_vals = graph_data[np.unravel_index(i_border, graph_data.shape)]
    j_border_vals = graph_data[np.unravel_index(j_border, graph_data.shape)]
...
def merge_weight(graph_rag: skimage._sk_graph.RAG, src: int, dst: int, neighbor: int,
    ...
    # Symmetric merge rule (same fix as edge_weight above).
    i_border_vals = graph_data[np.unravel_index(i_border, graph_data.shape)]
    j_border_vals = graph_data[np.unravel_index(j_border, graph_data.shape)]
...
def merge_regions(graph_rag: skimage._sk_graph.RAG, src: int, dst: int):
    """
...
def merge_nodes(graph_rag: skimage._sk_graph.RAG, src: int, dst: int, graph_data: np.ndarray):
...
def trim_region(graph_rag: skimage._sk_graph.RAG, labels_merged: np.ndarray, graph_data: np.ndarray, region_num: int,
                trim_volume: float):
...
def detect_tfpeaks(segment_data: np.ndarray, start_time=0, d_time=1, d_freq=1, merge_thresh=8, max_merges=np.inf,
    ...
    RAG = _sk_graph.RAG(join_labels, connectivity=2)
